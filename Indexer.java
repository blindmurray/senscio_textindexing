...
public class Indexer {
	...
	public static Document getDocument(File newFile, File oldFile) throws IOException {
		...
		try (InputStream stream = Files.newInputStream(newFile.toPath())) {
			...
			// Creates file Path field, extension field, concent field, and file
			// name field
			Field previewField = new StringField(LuceneConstants.FILE_PREVIEW, blurb, Field.Store.YES);
			Field filePathField = new StringField(LuceneConstants.FILE_PATH, oldFile.getAbsolutePath(),
					Field.Store.YES);
			Field extField = new TextField(LuceneConstants.FILE_EXT, TXT.getExtension(oldFile.toString()),
					Field.Store.YES);
	...
	public static Document getDocumentNot(File newFile, File oldFile) throws IOException {
		...
		try (InputStream stream = Files.newInputStream(newFile.toPath())) {
			...
			// Creates file Path field, extension field, concent field, and file
			// name field
			Field previewField = new StringField(LuceneConstants.FILE_PREVIEW, content, Field.Store.YES);
			Field filePathField = new StringField(LuceneConstants.FILE_PATH, oldFile.getAbsolutePath(),
					Field.Store.YES);
			Field extField = new TextField(LuceneConstants.FILE_EXT, TXT.getExtension(oldFile.toString()),
					Field.Store.YES);
	...
	public static void addIndex(String indexDir, String dataDir, Indexer indexer)
			throws IOException, TikaException, SAXException {
		indexer = new Indexer(indexDir);
		int numIndexed;
		long startTime = System.currentTimeMillis();
	...
	public static int writeIndex(String dataDirPath, FileFilter filter)
			throws IOException, TikaException, SAXException {
		...
		for (int i = 0; i < files.length; i++) {
			...
			// Will index file if file is a .txt file
					...
					&& TXT.getExtension(file.toString()).equals("txt")) {
				indexFile(file, file);
			}

			// Will convert file by calling Parse class if file is not .txt
			else if (TXT.getExtension(file.toString())
